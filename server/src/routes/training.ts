...
import { saveTrainingEntry, getTrainingEntries, getTrainingForSession } from '../services/firebase.js'
...
// GET /api/training — fetch training data from ALL sessions (for RL)
trainingRouter.get('/', async (req, res) => {
  try {
    const parsed = parseInt(req.query.limit as string)
    const limit = Number.isFinite(parsed) && parsed > 0 ? Math.min(parsed, MAX_LIMIT) : 50
...
trainingRouter.post('/', async (req, res) => {
  try {
    ...
    const {
      seasonNumber, seasonTheme, seasonPhase, totalScenes, castNames,
      scenes, relationships, couples, eliminatedIds, winnerCouple,
      dramaScores, viewerSentiment, casaAmorState,
      summary, seasonExport, rlExport,
