...
function describeRateLimit(rawMessage: string): string {
  ...
  if (isFreeTier) {
    let msg =
      "Hit the FREE TIER quota — your API key is on a Cloud project that does not have billing linked.";
    if (quotaId) msg += ` (quota: ${quotaId})`;
    msg +=
      " Link billing to that project, or generate a new key under your billed project.";
    return msg;
  }

  if (quotaId)
    return `Rate limit reached (quota: ${quotaId}). Wait a moment and try again.`;
...
export async function generateSceneFromGemini(
  ...
  if (
    ...
    finalMsg.toLowerCase().includes("resource_exhausted")
  ) {
