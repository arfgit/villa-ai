...
async function ensureInit(): Promise<void> {
  ...
  try {
    ...
    const serviceAccountPath = rawPath
      ? resolve(SERVER_ROOT, rawPath)
      : undefined;
    ...
    if (serviceAccountPath && existsSync(serviceAccountPath)) {
      ...
      console.log(
        "[firebase] no credentials found, using local JSON file storage",
      );
...
export async function saveSession(
  ...
  if (useFirestore && db) {
    ...
    localWrite("villaSessions", sessionId, {
      ...(data as Record<string, unknown>),
      updatedAt: Date.now(),
    });
  }
...
export async function saveTrainingEntry(
  ...
  if (useFirestore && db) {
    ...
    localWrite("trainingData", sessionId, {
      ...(data as Record<string, unknown>),
      updatedAt: Date.now(),
    });
  }
...
export async function addTrainingEntry(data: unknown): Promise<string> {
  ...
  localWrite("trainingData", entryId, {
    ...(data as Record<string, unknown>),
    id: entryId,
    createdAt: Date.now(),
  });
...
export async function getTrainingForSession(
  sessionId: string,
): Promise<unknown[]> {
  await ensureInit();
  if (useFirestore && db) {
  ...
  return localQueryAll("trainingData").filter(
    (d) => (d as Record<string, unknown>).sessionId === sessionId,
  );
}
