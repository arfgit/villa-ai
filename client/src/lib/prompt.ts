...
import { buildPastSeasonsPromptBlock } from "@/lib/trainingData";
import { VOICE_EXAMPLES } from "@/lib/castGenerator";
...
export interface RecouplePlanStep {
  ...
  partnerId: string;
  partnerName: string;
  rationale: string; // short phrase: "highest trust", "shared Casa Amor survival", etc.
}

export interface RecoupleScript {
  steps: RecouplePlanStep[];
  unpairedId?: string; // person left without a pick — going home
  unpairedName?: string;
}
...
export interface MinigameDefinition {
  ...
  rules: string; // 1-2 lines the host reads out
  winCondition: string;
}
...
interface BuildArgs {
  ...
  challengeCategory?: "learn_facts" | "explore_attraction";
  casaAmorCast?: Agent[];
