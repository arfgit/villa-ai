  ...
  return (
    <div className="border border-villa-pink/30 bg-villa-bg-2/40 p-3 h-full flex flex-col">
      <div className="text-[10px] uppercase tracking-widest text-villa-pink/70 mb-3 flex items-center justify-between">
        <span>
          ░ cast · {remaining}/{cast.length} ░
        </span>
        <Tooltip
          content="The villa contestants. Cast members left without a partner get eliminated. Last couple standing wins the season."
          side="bottom"
        >
      ...
      {winnerCouple && (
        <div className="mb-3 p-2 border border-villa-sun bg-villa-sun/10 text-center animate-villa-fadein">
          <div className="text-[9px] uppercase tracking-widest text-villa-sun mb-1">
            ★ winners ★
          </div>
          <div className="text-xs">
            {cast.find((c) => c.id === winnerCouple.a)?.name} &{" "}
            {cast.find((c) => c.id === winnerCouple.b)?.name}
          </div>
        </div>
      )}
      ...
      <div className="flex-1 overflow-y-auto scrollbar-thin space-y-3 pr-1">
        {sorted.map((agent) => {
          ...
          const locationLabel = location
            ? getSceneLabel(
                location,
                location === "recouple" && location === currentSceneType
                  ? recoupleOrdinal
                  : undefined,
              )
            : null;
          const isOnStage =
            location !== undefined && location === currentSceneType;
          const brain = brains[agent.id];
          const memCount = brain?.memories.length ?? 0;
          const reflectionCount =
            brain?.memories.filter((m) => m.type === "reflection").length ?? 0;
          ...
          const tooltipContent = (
            <div className="space-y-1.5 max-w-[280px]">
              <div className="text-villa-pink font-bold text-[10px] uppercase tracking-wider">
                {agent.name}'s brain
              </div>
              <div className="text-villa-dim italic">{agent.bio}</div>
              <div className="flex items-center gap-2 border-t border-villa-dim/30 pt-1">
                <span className="text-villa-dim uppercase text-[8px] tracking-wider">
                  reward
                </span>
                <span
                  className={clsx(
              ...
              {policy && (
                <div className="border-l-2 border-villa-aqua pl-1.5">
                  <span className="text-villa-aqua uppercase text-[8px] tracking-wider">
                    strategy
                  </span>
              ...
              {goal && (
                <div className="border-l-2 border-villa-sun pl-1.5">
                  <span className="text-villa-sun uppercase text-[8px] tracking-wider">
                    goal
                  </span>
              ...
              {recentRewards.length > 0 && (
                <div>
                  <div className="text-villa-dim uppercase text-[8px] tracking-wider mb-0.5">
                    recent rewards
                  </div>
                  <ul className="space-y-0.5">
                    {recentRewards.map((r) => (
                      <li
                        key={r.id}
                        className={clsx(
                          r.amount >= 0 ? "text-villa-aqua" : "text-villa-love",
                        )}
                      >
              ...
              {recentMems.length > 0 ? (
                <div>
                  <div className="text-villa-aqua uppercase text-[8px] tracking-wider mb-0.5">
                    recent memories ({memCount} total)
                  </div>
                  <ul className="space-y-1">
                    {recentMems.map((m) => (
                      ...
                      >
                        <span className="text-villa-dim">
                          [s{m.sceneNumber} ·{" "}
                          {m.type === "reflection" ? "✦" : "·"} i{m.importance}]
                        </span>{" "}
                        {m.content}
                      </li>
                    ))}
              ...
              {eliminated && (
                <div className="text-villa-love uppercase text-[8px]">
                  eliminated
                </div>
              )}
            </div>
            ...
            <Tooltip key={agent.id} content={tooltipContent} side="right">
              <div
                className={clsx(
                  ...
                  winner &&
                    "bg-villa-sun/5 px-2 py-1 border border-villa-sun/40",
                )}
              >
                ...
                <div className="flex-1 min-w-0 text-left">
                  ...
                  >
                    ...
                    {winner && (
                      <span className="text-villa-sun text-[9px]">★</span>
                    )}
                  </div>
                  <div className="text-[9px] text-villa-dim italic">
                    {agent.archetype}
                  </div>
                  {eliminated ? (
                    ...
                    <>
                      <div className="text-[9px] text-villa-dim mt-0.5">
                        feeling: {emotion}
                      </div>
                      {locationLabel && (
                        <div
                          className={clsx(
                            "text-[9px] mt-0.5",
                            isOnStage ? "text-villa-aqua" : "text-villa-dim",
                          )}
                        >
                          📍 {locationLabel.emoji}{" "}
                          {locationLabel.title.toLowerCase()}
                          {isOnStage && (
                            <span className="ml-1 text-villa-sun">●</span>
                          )}
                        </div>
                      )}
                      {(memCount > 0 || brain) && (
                      ...
                      {policy && (
                        <div
                          className="text-[9px] text-villa-aqua mt-0.5 truncate"
                          title={policy}
                        >
                          ⚡ {policy}
                        </div>
                      ...
                      {goal && (
                        <div
                          className="text-[9px] text-villa-sun mt-0.5 truncate"
                          title={goal}
                        >
                          ▸ {goal}
                        </div>
