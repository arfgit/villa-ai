...
export default function BottomActionBar({
  ...
  const startNextSeason = useVillaStore((s) => s.startNextSeason);
  ...
  const showRetry =
    lastError !== null && !isGenerating && sceneCount > 0 && !winnerCouple;
    ...
    <div className="border-t border-villa-pink/30 bg-villa-bg-2/60 backdrop-blur px-3 py-2 flex items-center gap-2 flex-wrap pb-[max(0.5rem,env(safe-area-inset-bottom))]">
      <div
        className={clsx(
          ...
          isGenerating
            ? "border-villa-pink/60 text-villa-pink opacity-100"
            : "border-transparent opacity-0 pointer-events-none absolute",
        )}
      >
        <div className="flex flex-col gap-1.5 w-full">
          <div className="flex items-center justify-between gap-2">
            <div className="flex items-center gap-2">
              <span
                className={clsx("inline-block", isGenerating && "animate-spin")}
              >
                ◐
              </span>
          ...
          <span
            className={clsx(
              "text-[8px] text-villa-dim normal-case tracking-normal",
              !generationProgress && "opacity-0",
            )}
          >
      ...
      {showStart && (
        <Tooltip
          content="Begin the season. Each islander introduces themselves, then the show plays out automatically scene by scene."
          side="top"
        >
          <button
          ...
          >
            <span
              className="inline-block w-0 h-0 border-l-[6px] border-l-current border-y-[4px] border-y-transparent"
              aria-hidden="true"
            />
      ...
      {showRetry && (
        <Tooltip
          content="The writers room hit an error. Try the last scene generation again."
          side="top"
        >
          <button
            onClick={() => generateScene()}
      ...
      {showNewSeason && (
        <>
          <Tooltip
            content="Start Season N+1 on this same session. Archives the finished season so it stays browsable; refreshes the villa with a new cast and new theme. Your share URL doesn't change."
            side="top"
          >
            <button
          ...
          <Tooltip
            content="Start over with a brand-new villa at a new share URL. Your current session stays preserved at its old URL — nothing is lost."
            side="top"
          >
            <button
              onClick={startNewVilla}
              className="inline-flex items-center justify-center gap-2 px-3 py-2 text-xs uppercase tracking-widest border border-villa-aqua text-villa-aqua hover:bg-villa-aqua hover:text-villa-bg cursor-pointer leading-none"
            >
              <span>⌂</span>
              <span>new villa</span>
            </button>
          ...
          <Tooltip
            content="Download season data as JSON for fine-tuning and reference."
            side="top"
          >
            <button
          ...
          <Tooltip
            content="Download RL agent training data (brains, rewards, memories)."
            side="top"
          >
            <button
      ...
      {showPause && (
        <Tooltip
          content={
            isPaused
              ? "Resume the show."
              : "Pause playback so you can read the dialogue at your own pace."
          }
          side="top"
        >
          <button
            ...
            className={clsx(
              ...
              isPaused
                ...
                : "border-villa-aqua text-villa-aqua hover:bg-villa-aqua/10",
            )}
          >
            {isPaused ? (
              <>
      ...
      <Tooltip
        content="Background 8 bit chiptune to set the villa mood."
        side="top"
      >
        <button
      ...
      <Tooltip
        content="Hide or show the help tooltips throughout the app."
        side="top"
      >
        <button
          ...
          className={clsx(
            "inline-flex items-center px-2 py-1.5 text-[10px] uppercase border cursor-pointer leading-none",
            tooltipsEnabled
              ? "border-villa-aqua text-villa-aqua"
              : "border-villa-dim/40 text-villa-dim hover:border-villa-dim",
          )}
        >
