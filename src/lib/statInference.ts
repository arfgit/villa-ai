...
type StatDelta = { trust: number; attraction: number; jealousy: number; compatibility: number }
...
const SHARED_SCENE_BASELINE: StatDelta = { trust: 0.4, attraction: 0.2, jealousy: 0, compatibility: 0.1 }
...
const ACTION_MODIFIERS: Array<{ match: RegExp; delta: StatDelta }> = [
  { match: /\b(kiss|kisses|kissed|snog)/i,      delta: { trust: 3, attraction: 8, jealousy: 0, compatibility: 0.5 } },
  { match: /\b(hug|hugs|hugged|embrace)/i,      delta: { trust: 4, attraction: 3, jealousy: 0, compatibility: 1.5 } },
  { match: /\b(laugh|laughs|grin|smile)/i,      delta: { trust: 1.5, attraction: 1.5, jealousy: 0, compatibility: 0.8 } },
  { match: /\b(flirt|wink|whisper)/i,           delta: { trust: 0.5, attraction: 4, jealousy: 0, compatibility: 0.2 } },
  { match: /\b(shout|yell|storm off)/i,         delta: { trust: -4, attraction: -1.5, jealousy: 3, compatibility: -1.5 } },
  { match: /\b(cry|sob|tear)/i,                 delta: { trust: 3, attraction: 0, jealousy: 0, compatibility: 1.2 } },
  { match: /\b(glare|side-eye|ignore|scoff)/i,  delta: { trust: -1.5, attraction: -1.5, jealousy: 1.5, compatibility: -0.8 } },
  { match: /\b(leans?|touch)/i,                 delta: { trust: 1.5, attraction: 3, jealousy: 0, compatibility: 0.3 } },
  { match: /\b(whisper|lean close|pull aside)/i, delta: { trust: 0, attraction: 2, jealousy: 3, compatibility: 0.5 } },
  { match: /\b(grab|hold hands|interlock)/i,    delta: { trust: 2, attraction: 3, jealousy: 2, compatibility: 0.8 } },
]

const INFERENCE_STRENGTH = 1.2

const PER_SCENE_CAP = { trust: 8, attraction: 8, jealousy: 10, compatibility: 4 }
...
export function inferStatDeltas(
  ...
  function bump(from: string, to: string, d: StatDelta) {
    if (from === to) return
    const key = `${from}->${to}`
    const cur = acc.get(key) ?? { trust: 0, attraction: 0, jealousy: 0, compatibility: 0 }
    cur.trust += d.trust
    ...
    if (target) {
      ...
      if (jealousyWeight > 0) {
        ...
        for (const observer of participantIds) {
          ...
          if (observer === speakerPartner || observer === targetPartner) continue
          const obsToSpeaker = rels.find((r) => r.fromId === observer && r.toId === line.agentId)
          const obsToTarget = rels.find((r) => r.fromId === observer && r.toId === target)
          if ((obsToSpeaker?.attraction ?? 0) > 35) {
            bump(observer, target, { trust: 0, attraction: 0, jealousy: 3 * jealousyWeight, compatibility: 0 })
          }
          if ((obsToTarget?.attraction ?? 0) > 35) {
            bump(observer, line.agentId, { trust: 0, attraction: 0, jealousy: 3 * jealousyWeight, compatibility: 0 })
          }
        }
      }
    } else {
  ...
  for (const c of couples) {
    if (participantIds.includes(c.a) && participantIds.includes(c.b)) {
      bump(c.a, c.b, { trust: 0.8, attraction: 0.5, jealousy: -0.3, compatibility: 0.5 })
      bump(c.b, c.a, { trust: 0.8, attraction: 0.5, jealousy: -0.3, compatibility: 0.5 })
    }
  }
  ...
  for (const [key, d] of acc.entries()) {
    ...
    const compatibility = Math.round(clamp(d.compatibility * INFERENCE_STRENGTH, -PER_SCENE_CAP.compatibility, PER_SCENE_CAP.compatibility))
    if (trust === 0 && attraction === 0 && jealousy === 0 && compatibility === 0) continue
