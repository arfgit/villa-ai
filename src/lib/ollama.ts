...
export async function generateSceneFromOllama(
  ...
  const host = (import.meta.env.VITE_OLLAMA_HOST as string | undefined) ?? DEFAULT_HOST
  const model = (import.meta.env.VITE_OLLAMA_MODEL as string | undefined) ?? DEFAULT_MODEL
  ...
  throw lastError instanceof Error ? lastError : new Error('Ollama generation failed')
}
...
export async function generateBatchFromOllama(
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    ...
    try {
      return parseAndValidateBatch(data.response, validAgentIds, requiredSpeakerIds)
    } catch (err) {
      lastError = err
    }
  }

  throw lastError instanceof Error ? lastError : new Error('Ollama batch generation failed')
}
