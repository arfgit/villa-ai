...
export interface DialogueLine {
  ...
  intent?: TurnIntent; // added by Scene Engine — what this line is DOING
  beatIndex?: number; // which planned beat this line is satisfying
  quotable?: boolean; // LLM-tagged: this line is spicy enough for viewer-chat to quote
}

// Vocabulary the scene engine + LLM use to tag what each line is doing.
// Small enough that coercion is easy, broad enough to cover push/pull drama.
...
export interface Subtext {
  surface: string; // what they'll say on the surface
  actual: string; // what they actually want/mean
}
...
export interface PlannedBeat {
  speakerId: string;
  intent: TurnIntent;
  emotionalTone: string; // free-form: "cold politeness", "barely-held-together calm"
  target?: string; // agentId the beat is aimed at
  loud?: boolean; // true when this beat is the peak — CAPS + shake earned
}

export interface SceneContext {
  sceneType: SceneType;
  tension: number; // 0–100
  powerDynamic: string; // 1-line summary of who holds leverage
  recentEvent: string; // the thing this scene is reacting to
  pattern: DialoguePattern;
  plannedBeats: PlannedBeat[];
  roles: PerAgentSceneRole[];
  callbackHooks: string[]; // quoted fragments from prior scenes (pass 2)
}
...
export interface Scene {
  ...
  sceneContext?: SceneContext; // Scene Engine plan for this scene (debug + RL replay)
}
...
// v2 adds scene.sceneContext (the Scene Engine plan) for each scene so
// downstream analyses / RL training can attribute deltas to planned beats.
// Loaders tolerate v1 payloads by treating sceneContext as undefined.
